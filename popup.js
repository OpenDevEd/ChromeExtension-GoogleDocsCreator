...
$(function () {
	...
	$('#template').change(function () {
		...
		if (/^[a-zA-Z0-9_-]+$/.test(templateID) === false) {
			const regex = /\/d\/([a-zA-Z0-9_-]+)/;
			const match = templateID.match(regex);
			if (match && match[1]) {
				templateID = match[1];
			} else {
				templateID = '';
...
function getDetailsText(href, titlePrefix, authuserName, TID) {
	...
	if (href.match(longre)) {
		...
		if (found) {
			...
			detailsPanel += '<p>Source: ' + linkster('Google drive folder', href) + '</p>\n';
		...
		detailsPanel += '<p>Source: ' + linkster('Google doc/sheet/slide', href) + '</p>\n';
	} else {
	};
	return detailsPanel;
};
	...
	if (href.match(longre)) {
		...
		if (href.match('/document|spreadsheets/')) {
			...
			str += '<p>Additional formats (jpg, png, svg) cannot be downloaded by ' +
				linkster("an extension", "https://stackoverflow.com/questions/31662455/how-to-download-google-slides-as-images")
				+ '</p>';
			// Poss via apps script: https://stackoverflow.com/questions/31662455/how-to-download-google-slides-as-images
		} else if (href.match('/drawings/')) {
			type = 'drawings';
			formats = ['pdf', 'svg'];
			for (var i = 0; i < formats.length; i++) {
				var thislink = 'https://docs.google.com/' + type + '/d/' + FILE_ID + '/export/' + formats[i] + '?' + auStr;
		...
		$('#optionspanel').empty();
		$('#options').empty();
		str += '<div id="error">This is extension is designed to run on Google Drive / Docs / Sheets /  Slides / Forms pages. Please visit a folder in ' +
			linkster("Google Drive", "https://drive.google.com") + ' or a Google Docs / Sheets / Slides / Forms and try again.</div>';
	};
	return str;
};
